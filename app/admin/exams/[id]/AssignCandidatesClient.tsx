        ...
        <div>
            ...
            <div className="mb-4">
                ...
                <div className="space-y-3">
                    {/* Option 1: Slot Selection Guide */}
                    <div className={`p-3 rounded border cursor-pointer transition-colors ${!scheduledTime ? 'bg-blue-50 border-blue-500 ring-1 ring-blue-500' : 'bg-gray-50 border-gray-200 hover:bg-gray-100'}`}
                    ...
                    >
                        <div className="flex items-center">
                            ...
                            />
                            <div className="ml-3">
                                <span className="block text-sm font-medium text-gray-900">Let Candidates Choose Slot</span>
                                <span className="block text-xs text-gray-500">Candidates pick from available slots in their dashboard.</span>
                            </div>
                        </div>
                    </div>

                    {/* Option 2: Specific Time */}
                    <div className={`p-3 rounded border transition-colors ${scheduledTime ? 'bg-blue-50 border-blue-500 ring-1 ring-blue-500' : 'bg-gray-50 border-gray-200'}`}>
                        <div className="flex items-center mb-2" onClick={() => !scheduledTime && setScheduledTime(new Date().toISOString().slice(0, 16))}>
                            <input
                                type="radio"
                                checked={!!scheduledTime}
                                readOnly
                            ...
                            />
                            <div className="ml-3">
                                <span className="block text-sm font-medium text-gray-900">Force Specific Time</span>
                            </div>
                        </div>
                        <input
                            type="datetime-local"
                            className={`w-full border rounded p-2 text-sm ${!scheduledTime ? 'opacity-50 pointer-events-none bg-gray-100' : 'bg-white'}`}
                            value={scheduledTime}
                            onChange={(e) => setScheduledTime(e.target.value)}
                        />
                        <p className="text-xs text-gray-500 mt-1 pl-7">Overrides slot system. Exam starts exactly at this time.</p>
                    </div>
                </div>
            </div>
